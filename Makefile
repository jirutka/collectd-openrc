...
OPENRC_CFLAGS  = $(shell $(PKG_CONFIG) --cflags openrc)
OPENRC_LIBS    = $(shell $(PKG_CONFIG) --libs openrc)
...
CFLAGS        += -std=c11 -fPIC -I$(COLLECTD_INCLUDE_DIR) $(OPENRC_CFLAGS) $(JSONC_CFLAGS)
LDFLAGS       += -shared
LIBS          += $(OPENRC_LIBS) $(JSONC_LIBS)
